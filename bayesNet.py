...
class BayesNet(object):
    ...
    def __init__(self, variables, inputInEdges, inputOutEdges, inputVariableDomains):
        """
 贝叶斯网的裸骨构造函数。
        使用 constructEmptyBayesNet 获得更好的界面。

variables：所有变量的可迭代对象。
        inEdges：将变量映射到 otherVariable 的字典
                         当有一条从 otherVariable 到
                         变量
        outEdges：将变量映射到 otherVariable 的字典
                         其中有一个从变量到
                         其他变量
        variableDomains：将每个变量映射到其域的字典（类似列表）。

构造一个贝叶斯网络，其边由 inEdges 和
        outEdges 为每个变量。
        不初始化任何变量的条件概率表。
        """
