...
def constructBayesNet(gameState: hunters.GameState):
    """
  根据图1中给出的结构构建一个空的贝叶斯网
    项目描述。

您*必须*使用此函数中的常量命名所有变量。

在此方法中，您应该：
    - 使用 Bayes Net 节点填充“变量”
    - 用贝叶斯网络中的每条边填充“边”。我们将代表每个
      edge 作为元组 '（from， to）'。
    - 设置每个 'variableDomainsDict[var] = values'，其中 'values' 是一个列表
      对 'var' 的可能赋值。
        - 每个代理位置都是一个元组 （x， y），其中 x 和 y 索引为 0
        - 每个观测距离都是嘈杂的曼哈顿距离：
          它是非负面的，并且 |obs - true|<= MAX_NOISE
    - 这使用稍微简化的机制，而后来为了简单起见而使用的机制
    """
    # constants to use
    PAC = "Pacman"
    GHOST0 = "Ghost0"
    GHOST1 = "Ghost1"
    OBS0 = "Observation0" # 代理位置
    OBS1 = "Observation1"
    X_RANGE = gameState.getWalls().width
    Y_RANGE = gameState.getWalls().height
    MAX_NOISE = 7

    variables = [] # list of variables，节点，节点名称
    edges = [] # list of edges，边
    variableDomainsDict = {} # 变量的取值范围
    ...
    ManHanDistance = X_RANGE+Y_RANGE+MAX_NOISE-1
    variableDomainsDict[OBS0] = [x for x in range(ManHanDistance)]
    variableDomainsDict[OBS1] = [x for x in range(ManHanDistance)]
...
def inferenceByEnumeration(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict):
    ...
    # 初始化返回变量和要消除的变量
    evidenceVariablesSet = set(evidenceDict.keys()) # 被观察的变量
    queryVariablesSet = set(queryVariables)# 查询的变量
    eliminationVariables = (bayesNet.variablesSet() - evidenceVariablesSet) - queryVariablesSet# 要消除的变量
    
    # 获取我们知道证据变量的所有因子（以减小表的大小）
    ...
    # currentFactorsList 现在应该包含图形的连接组件作为因子，必须连接连接的组件
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        joinFactorsByVariable = joinFactorsByVariableWithCallTracking(callTrackingList)
        eliminate             = eliminateWithCallTracking(callTrackingList)
        if eliminationOrder is None: # 如果给定 None ，则设置任意消除顺序
        ...
        # 按eliminationOrder的顺序连接表
        for joinVariable in eliminationOrder:
            # 通过变量连接所有因子
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)
            # 如果该因子仅包含一个非条件变量，则将其丢弃,否则将其连接
    ...
    def normalize(self):
        ...
        if self.total() != 0:
            ...
            for key in self.keys():
                new_key_val[key] = self[key] / self.total()
            for key in self.keys():
                self[key] = new_key_val[key]
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            obs_prob = self.getObservationProb(observation, pacman_pos, pos, jail_pos)

            new_beliefs[pos] = prior * obs_prob
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            ...
            old_prob = self.beliefs[pos]
            if old_prob ==0 :  continue

            now_prob = self.getPositionDistribution(gameState, pos)

            for new_pos, prob in now_prob.items():
                beliefs[new_pos] += old_prob * prob
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        beliefs = DiscreteDistribution()
        for particle in self.particles:
            beliefs[particle] += self.getObservationProb(observation, pacPos, particle, jailPos)
        if beliefs.total() == 0 :
            self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        newParticles = []
        for particle in self.particles:
            transitionDist = self.getPositionDistribution(gameState, particle)
